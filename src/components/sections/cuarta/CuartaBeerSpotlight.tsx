...
export default function CuartaBeerSpotlight() {
  ...
  return (
    <section id="cuarta-spotlight" style={{ background: '#080507', padding: '80px 0 60px', overflow: 'hidden' }}>
      {/* Section header */}
      <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '0 24px', marginBottom: '64px' }}>
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px' }}>
            <div style={{ width: '48px', height: '1px', background: '#C8921C' }} />
      ...
      <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '0 24px' }}>
        {canillas.map((beer: any, i: number) => (
          <motion.div
            ...
            style={{
              gap: '0',
              borderBottom: i < canillas.length - 1 ? '1px solid rgba(200,146,28,0.08)' : 'none',
              padding: '40px 0',
              alignItems: 'center',
            }}
          >
            ...
            <div style={{ order: i % 2 === 0 ? 0 : 1, paddingRight: i % 2 === 0 ? '40px' : '0', paddingLeft: i % 2 !== 0 ? '40px' : '0' }}>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: '16px', marginBottom: '12px' }}>
                <span style={{ fontFamily: "'Bebas Neue', cursive", fontSize: '80px', color: 'rgba(200,146,28,0.08)', lineHeight: 1 }}>
                  {String(i + 1).padStart(2, '0')}
                </span>
                <div>
                  <div style={{ fontFamily: "'Bebas Neue', cursive", fontSize: 'clamp(28px, 3vw, 44px)', color: '#F0E2BF', lineHeight: 1, letterSpacing: '0.04em' }}>
                    {beer.nombre}
                  </div>
                  {beer.badges?.length > 0 && (
                    <span style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: '10px', letterSpacing: '0.3em', color: '#C8921C', textTransform: 'uppercase', background: 'rgba(200,146,28,0.1)', padding: '2px 8px' }}>
                      {beer.badges[0]}
                    </span>
                  )}
                </div>
              </div>
              <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: '17px', color: '#9E8050', marginBottom: '16px', lineHeight: 1.5, maxWidth: '380px' }}>
                {beer.desc}
              </p>
              <div style={{ fontFamily: "'Bebas Neue', cursive", fontSize: '28px', color: '#C8921C', letterSpacing: '0.04em' }}>
            ...
            <div className="hidden lg:flex" style={{ order: i % 2 === 0 ? 1 : 0, alignItems: 'center', justifyContent: 'center' }}>
              <motion.div
                ...
                style={{
                  ...
                  background: `radial-gradient(circle, rgba(200,146,28,0.12) 0%, rgba(8,5,7,0.8) 70%)`,
                  border: '1px solid rgba(200,146,28,0.15)',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  position: 'relative',
                }}
              >
                <Beer size={52} style={{ color: '#8B6010', opacity: 0.7 }} strokeWidth={1.2} />
                {/* Amber glow ring */}
                <div style={{
                  position: 'absolute', inset: '-12px', borderRadius: '50%',
                  border: '1px solid rgba(200,146,28,0.06)',
                }} />
              </motion.div>
            </div>
          </motion.div>
      ...
      >
        <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: '18px', color: '#9E8050', letterSpacing: '0.05em' }}>
          Las canillas rotan semanalmente. Consultá disponibilidad.
        </p>
        <a
          href="#cuarta-menu"
          style={{
            border: '1px solid rgba(200,146,28,0.3)', color: '#F0E2BF',
            padding: '12px 28px',
